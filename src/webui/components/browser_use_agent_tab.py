...
async def run_agent_task(
    ...
    vnc_view_comp = webui_manager.get_component_by_id("browser_use_agent.vnc_view")
    ...
    # --- Browser Settings (needed early for UI toggle) ---
    ...
    yield {
        ...
        vnc_view_comp: gr.update(visible=not headless),
        browser_view_comp: gr.update(visible=headless),
    }
    ...
    # --- 3. *** CRITICAL FIX: VALIDATE LLM INITIALIZATION *** ---
    if not main_llm:
        gr.Error("LLM failed to initialize. Please check your LLM Provider and Model Name in the 'Agent Settings' tab.")
        # Reset UI to be interactive again
        yield {
            user_input_comp: gr.update(interactive=True, placeholder="Enter your task..."),
    ...
    try:
        ...
        while not agent_task.done():
            ...
            # Update Browser View (Screenshot for headless)
            if headless and webui_manager.bu_browser_context:
                try:
                    ...
                    if screenshot_b64:
                        html_content = f'<div style="display:flex; justify-content:center; align-items:center; height:100%;"><img src="data:image/jpeg;base64,{screenshot_b64}" style="max-width: 100%; max-height: 100%; object-fit:contain; border:1px solid #ccc;"></div>'
                        update_dict[browser_view_comp] = gr.update(value=html_content)
                    else:
                        html_content = f"<div style='display:flex; justify-content:center; align-items:center; height:100%;'><h1>Waiting for browser session...</h1></div>"
                        update_dict[browser_view_comp] = gr.update(value=html_content)
                except Exception as e:
                    logger.debug(f"Failed to capture screenshot: {e}")
                    update_dict[browser_view_comp] = gr.update(
                        value="<div style='display:flex; justify-content:center; align-items:center; height:100%;'>Error loading view...</div>"
                    )
...
async def handle_clear(webui_manager: WebuiManager):
    ...
    return {
        ...
        webui_manager.get_component_by_id("browser_use_agent.browser_view"): gr.update(
            value="<div style='display:flex; justify-content:center; align-items:center; height:100%;'>Browser Cleared</div>",
            visible=False,
        ),
        webui_manager.get_component_by_id("browser_use_agent.vnc_view"): gr.update(
            visible=True
        ),
...
def create_browser_use_agent_tab(webui_manager: WebuiManager):
    """
    Create the run agent tab with animated browser, fixed chatbot wrapping, and styled headers.
    """
    webui_manager.init_browser_use_agent()

    # --- 1. Define the CSS for all UI enhancements ---
    ui_enhancements_css = """
    <style>
    #browser_column {
        transition: max-width 0.5s ease-in-out, min-width 0.5s ease-in-out, opacity 0.4s ease-in-out,
                    margin 0.5s ease-in-out, padding 0.5s ease-in-out;
        max-width: 50vw; opacity: 1; overflow: hidden;
    }
    #browser_column.browser-hidden {
        max-width: 0 !important; min-width: 0 !important; opacity: 0 !important;
        padding: 0 !important; margin: 0 !important; border-width: 0 !important;
    }
    #browser_use_chatbot .message-bubble .prose {
        white-space: pre-wrap !important; word-break: break-word !important;
    }
    .section-header {
        background-color: var(--neutral-800, #2b2f38); color: var(--body-text-color, white);
        padding: 8px 12px; border-radius: 8px; font-weight: 600;
        margin-bottom: 8px; border: 1px solid var(--neutral-700, #414853);
    }
    </style>
    """
    ...
    with gr.Row(equal_height=False):
        ...
        with gr.Column(scale=1):
            ...
            with gr.Row():
                ...
                toggle_view_button = gr.Button("🗂️ Hide Browser", interactive=True, variant="secondary", scale=2)
                run_button = gr.Button("▶️ Submit Task", variant="primary", scale=3)

            with gr.Column(elem_classes="task-outputs"):
                gr.HTML("<div class='section-header'>Task Outputs</div>")
                agent_history_file = gr.File(label="Agent History JSON", interactive=False)
                recording_gif = gr.Image(label="Task Recording GIF", format="gif", interactive=False, type="filepath")

        # --- Right Column: Browser View ---
        browser_column = gr.Column(scale=1, elem_id="browser_column")
        with browser_column:
            browser_view_md = gr.HTML("<div class='section-header'>Browser View</div>")
            vnc_view = gr.HTML(
                """<div style="width: 100%; height: 700px; border: 1px solid #333; display: flex; justify-content: center; align-items: center; background-color: #141414;">
                    <p style="color: #9ca3af;">Initializing VNC Viewer...</p>
                </div>""",
                label="VNC Browser View",
                elem_id="vnc_view",
                visible=True,
            )
            browser_view = gr.HTML(
                value="<div style='width:100%; height:700px; display:flex; justify-content:center; align-items:center; border:1px solid #333; background-color:#141414;'><p>Browser View (Active in Headless Mode)</p></div>",
    ...
    tab_components.update(
        dict(
            chatbot=chatbot, user_input=user_input, clear_button=clear_button, run_button=run_button,
            stop_button=stop_button, pause_resume_button=pause_resume_button, agent_history_file=agent_history_file,
            recording_gif=recording_gif, browser_view=browser_view, vnc_view=vnc_view,
            toggle_view_button=toggle_view_button, browser_view_md=browser_view_md,
        )
    )
    webui_manager.add_components("browser_use_agent", tab_components)
    webui_manager.browser_column = browser_column

    def _toggle_browser_view():
        current_hidden_state = getattr(webui_manager, "bu_browser_hidden", False)
        new_hidden_state = not current_hidden_state
        webui_manager.bu_browser_hidden = new_hidden_state
        browser_col = webui_manager.browser_column
        cb = webui_manager.get_component_by_id("browser_use_agent.chatbot")
        tb = webui_manager.get_component_by_id("browser_use_agent.toggle_view_button")
        if new_hidden_state:
            return {
                browser_col: gr.update(elem_classes=['browser-hidden']),
                cb: gr.update(height=900),
                tb: gr.update(value="🖥️ Show Browser"),
            }
        else:
            return {
                browser_col: gr.update(elem_classes=[]),
                cb: gr.update(height=600),
                tb: gr.update(value="🗂️ Hide Browser"),
            }
    
    toggle_view_button.click(
        fn=_toggle_browser_view,
        inputs=[],
        outputs=[
            browser_column,
            webui_manager.get_component_by_id("browser_use_agent.chatbot"),
            webui_manager.get_component_by_id("browser_use_agent.toggle_view_button"),
        ],
    )